...
strand_info_p = "individual_plus"
strand_info_m = "individual_minus"
...
for row in f.readlines():
	...
	if initial_columns[0]==columns[0] and int(columns[2])<=int(initial_columns[2])+int(mean_frag_length):
		print (columns[0],columns[1],columns[2],columns[3],strand_info_p+str(tag))
...
for row in f.readlines():
	columns = row.split()
	if initial_columns[0]==columns[0] and int(columns[2])<=int(initial_columns[2])+int(mean_frag_length):
		print (columns[0],columns[1],columns[2],columns[3],strand_info_m+str(tag))
		initial_columns=columns
	elif initial_columns[0]==columns[0] and int(columns[2])>int(initial_columns[2])+int(mean_frag_length):
		tag=tag+1
		print (columns[0],columns[1],columns[2],columns[3],strand_info_m+str(tag))
